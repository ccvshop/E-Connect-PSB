...
$config = Psb\Configuration::getDefaultConfiguration();
...
$meApi = new Psb\Api\MeApi(
    $httpClient,
    $config
);

$hookApi = new Psb\Api\HookApi(
    $httpClient,
    $config
);

$authn = new Psb\Authentication($config);

$salesInvoiceApi = new Psb\Api\SalesInvoiceApi(
    $httpClient,
    $config
);
...
<body>
    <div>
        ...
        <?php } else {
            ...
            Hello <?php print $userResponse['name']; ?> your access token is
            <pre><?php print $token; ?></pre>
            ...
            <form method="POST" enctype="multipart/form-data">
                <p>
                    ...
                    <input type="text" name="senderPartyId" value="<?php print $senderPartyId; ?>" />
                </p>
                <p>
                    <label>Receiver partyId:</label>
                    <input type="text" name="receiverPartyId" value="<?php print $receiverPartyId; ?>" />
                </p>
            ...
            if (isset($_POST['queryRecipient'])) {
                print '<pre>' . $salesInvoiceApi->queryRecipientPartyForSalesInvoice($_POST['senderPartyId'], [$_POST['receiverPartyId']], null) . '</pre>';
            } ?>
            ...
            <?php
            if (isset($_POST['send'])) {
                print '<pre>' . $salesInvoiceApi->sendSalesInvoice($_POST['senderPartyId'], $_FILES['file']['tmp_name'], $_POST['receiverPartyId']) . '</pre>';
            } ?>
